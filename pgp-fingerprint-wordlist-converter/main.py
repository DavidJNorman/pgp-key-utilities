...
def main(argv):
    ...
    def even(fingerprint):
        return wordlist[fingerprint][0]
    ...
    with open(str(argv[0]), 'r') as input_file, open(("io/%s" % (str(argv[1]))), "w+") as output_file:
        for line in input_file:
            ...
            output_line = ""
            is_even = True
            while len(processed_line) >= 2:
                print("Processing " + processed_line)
                if is_even:
                    output_line += (even(int(processed_line[:2], 16)) + ' ')
                    is_even = False
                else:
                    output_line += (odd(int(processed_line[:2], 16)) + ' ')
                    is_even = True
